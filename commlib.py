...
def pam_waveform1(ak, TS,p_callable = square, 
    ...
    for a,k in enumerate(ak):
        print('Egg thelo na pao apo :', (a)*samples,(a+1)*samples)
        x[(a)*samples:(a+1)*samples]=  p_callable( t[(a)*samples:(a+1)*samples] - k * TS, (a+1)*TS )
    
    
    return t, x

# pulse amplitude modulation : fast version
def pam_waveform2(ak, TS, M, 
                 samples = 10, tinitial = 0, tguard = 0.0):
    ...
    i = np.floor( (t - tinitial)*M/ TS).astype(int)
...
def plot_pam(t, x, M, bits, TS, tstart = 0, dy = 0, plot_type = '-'):
    ...
    while ( i < Nbits ) or ( tcurrent <= np.max(t) ):
       ...
       plt.text(tcurrent, xcurrent + dy, key,rotation=30)
       i += m
       tcurrent += TS
